...
class Json2DartLegacyCommand extends Command {
  ...
  @override
  String get name => 'json2dart-legacy';

  @override
  String get description => '(Deprecated) Generate dart data class from json.';
      ...
      if ((isOnlyUnitTest || isUnitTest) && body != null && response != null) {
        final result = createModelUnitTest(
          ...
          jsonBody: pathBody != null
              ...
              : '{}',
          jsonResponse:
              pathResponse != null ? read(pathResponse).join('\n') : '{}',
          body: bodyUnitTest,
...
${blocsName.map((e) => '        BlocListener<${e.pascalCase}Bloc, ${e.pascalCase}State>(listener: listener${e.pascalCase}Bloc,),').join('\n')}
  ...
  Future<void> handleApi({
    ...
    if (isApi) {
      printMessage(
          'morpheme api $apiName -f $featureName -p $pageName -r $returnData --method=$method --path=$pathUrl ${header != null ? '--header=$header' : ''} $argBody $argResponse $argCacheStrategy $argAppsName');
      await 'morpheme api $apiName -f $featureName -p $pageName -r $returnData --method=$method --path=$pathUrl ${header != null ? '--header=$header' : ''} $argBody $argResponse $argCacheStrategy $argAppsName'
          .run;
    }
  }
  ...
  dynamic getMapFromJson(
    ...
    final json = path != null ? read(path).join('\n') : '{}';
    dynamic response;
    try {
