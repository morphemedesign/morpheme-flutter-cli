...
class PatchAndroidCommand extends Command {
  ...
  @override
  String get description => 'Shorebird patch android with flavor.';
  ...
  void run() async {
    ...
    final command =
        'shorebird patch android -t $argTarget ${dartDefines.join(' ')} $argBuildNumber $argBuildName $argSplitDebugInfo $releaseVersion $argFlutterVersion --no-confirm -- $argObfuscate';
    printMessage(command);

    await command.run;

    StatusHelper.success('shorebird patch android');
  }
}
